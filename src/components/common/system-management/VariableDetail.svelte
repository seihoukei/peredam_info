...
<div class="centered flex" transition:slide>
    ...
    {#if inputType === "NUM"}
        <input class="large spacy-below" type="number" bind:value bind:this={element} placeholder={old}
               on:keydown={checkKey}/>
    {:else}
        <input class="large spacy-below" type="text" bind:this={element} bind:value placeholder={old}
               on:keydown={checkKey}/>
    {/if}
</div>
