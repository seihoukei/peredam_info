const HELP_PAGES = {
    ...
    Чтобы посмотреть информацию об имеющейся системе или переать показания - нажмите на кнопку соответствующей системы.
    ...
    Для передачи данных счетчиков нажмите "Передать". При этом если вы уже передавали данные в этом периоде или сейчас не рекомендвоанное поставщиком услуг время - сервис сообщит вам об этои.
    Для правки данных нажмите "Изменить".
    Кнопка "Назад" вернёт вас в список систем.
    `,
    
    'user/sys/edit' : `
    Чтобы сохранить изменения - нажмите "Сохранить". Если кнопка недоступна - провеерьте, все ли обязательные поля корректно заполнены.
    ...
    Когда все поля будут корректно заполнены, станут доступны кнопки для отправки показаний.
    В полях, где вы ещё не вводили значения, более прозрачными цифрами указаны прошлые показания, если вы передавали их ранее.
    Если поставщик услуг предоставил такую возможность, сервис отправит сообщение сам. В противном случае вам будут представлены способы, которыми можно передать показания самому.
    Если возможна отправка письмом или SMS-сообщением, вам так же будет предложена ссылка, нажав на которую вы сможете отправить заранее заготовленное письмо или сообщение. Обращаем внимание, что в таком случае за эту операцию может списываться оплата вашим оператором интернета или сотовой связи.
    ...
    'login/login' : `
    Укажите свой номер телефона или имя пользователя для идентификации. Если вы здесь впервые и не хотите указывать номер телефона - нажмите "Вход без телефона" и введите какое-нибудь имя пользователя и нажмите вперёд. Если имя свободно - вам будет предложено зарегистрироваться в системе.
    Вы можете также передать показания без регистрации, нажав кнопку "Просто передать показания". Основое отличие пользования сервисом с регистрацией - в том, что после разовой настройки можно получать уведосмления-напоминания о сроках передачи показаний и не вводить лицевые счета и номера счетчиков каждый раз.
    `,
    ...
    Также вам следует ознакомиться с простыми правилами пользования сервера. Самое главное - не злоупотребляйте удобством и простотой, чтобы заваливать операторов лишними сообещниями, передавайте сообщения в срок и один раз за период, если только не случилось что-то из ряда вон выходящее.
    ...
    'login/password/new' : `
    Придумайте пароль, чтобы другой человек не смогу зайти от вашего имени и всё испортить. Постарайтесь запомнить или зписать пароль, потому что в случае его утраты восстановить доступ может быть проблематично. Наши операторы вам помогут, но нас мало и у нас лапки, так что это займёт некоторое время.
    `,
    ...
    'login/code/set' : `
    Вы можете установить код быстрого доступа, чтобы защитить быстрый вход в систему, например, от случайных манипуляций детей или любого другого человека, получившего доступ к вашему устройтву. Код действует только на этом устройстве.
